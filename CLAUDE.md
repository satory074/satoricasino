...
- **AdSense must not render on the Auth view.** Google's "Google-served ads on screens without publisher-content" policy explicitly bans ads on login/registration screens. `App.tsx` early-returns the bare `<Auth>` component when `view === "auth"` — do **not** wrap Auth in `<SideAds>` or render `<BannerAd>` inside `Auth`. The `adsbygoogle.js` script in `index.html` is fine to load on Auth (it's just a script tag), but any `<ins class="adsbygoogle">` slot mounted while no real publisher content is on screen will trip the policy and put the site back into "要確認".
