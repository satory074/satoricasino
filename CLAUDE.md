...
├── auth/Auth.tsx          # Login/register card + crawler-visible footer links to info pages
├── info/InfoPage.tsx      # Public Privacy / Terms / About / Responsible Gaming pages (data-driven from i18n)
...
- **Public info routes bypass the passphrase gate.** Privacy Policy, Terms of Service, About, and Responsible Gaming live at `/privacy`, `/terms`, `/about`, `/responsible-gaming` and are rendered by `frontend/src/info/InfoPage.tsx` outside the auth gate. AdSense crawlers cannot get past the `PASSPHRASE` env, so these pages are the only "publisher content" Google can index — they must remain public. Routing is SPA-internal: `App.tsx` reads `window.location.pathname` on mount to set the initial view and uses `window.history.replaceState` to keep the URL in sync. `firebase.json` already rewrites `**` to `/index.html`, so any new info route works without hosting config changes. Page bodies are i18n-driven (`info.{privacy,terms,about,responsible}.sections.*.body` in `locales/ja.ts` and `en.ts`); adding a section means updating both locale files and the section list inside `InfoPage.tsx`.
